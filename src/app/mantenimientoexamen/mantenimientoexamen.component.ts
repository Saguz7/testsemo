...
`;
@Component({
  selector: 'app-mantenimientoexamen',
  templateUrl: './mantenimientoexamen.component.html',
  styleUrls: ['./mantenimientoexamen.component.css']
})
...
////////////////////////////////////
 ...
 ///////////////////
  ...
  mensajedeerror(licencia: any,event: any){
   ...
   habilitarbotones(examenaverificar: any,event: any){
      this.examenaux = examenaverificar;
          this.btneditarexamen = true;

                 if(event.target.tagName == "TD"){
                   for(var i = 1; i < event.target.parentNode.parentNode.parentNode.rows.length; i++){
                     event.target.parentNode.parentNode.parentNode.rows[i].style.backgroundColor = null;
                     }
